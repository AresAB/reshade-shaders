...
uniform float Speed < __UNIFORM_SLIDER_FLOAT1
	ui_label = "Hatch Direction Speed";
	ui_min = 0.; ui_max = 0.01;
	ui_tooltip = "The speed in which the crosshatch direction changes.";
> = 0.004;
...
float3 MyPass(float4 vois : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	if (hatch_index <= 1) return float3(1., 1., 1.);
		...
		[loop]
        for (int i = 1; i < hatch_index; i++){
            float angle = (3.14 * i * 1.618) % 1 * (round((cos(time * Speed) + 1.) * 0.5) * 2. - 1.);
